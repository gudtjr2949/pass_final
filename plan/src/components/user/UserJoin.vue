      ...
      <div class="form-group">
        ...
        <div class="input-container">
          <input
            ...
            ref="user_id"
            v-model="user_id"
            @input="checkDuplicateId"
          />
          <span
            class="status-message"
            v-if="idCheckResult === true"
            style="color: blue"
            >사용 가능</span
          >
          <span
            class="status-message"
            v-if="idCheckResult === false"
            style="color: red"
            >사용 불가능</span
          >
        </div>
...
export default {
  ...
  methods: {
    checkValue() {
      ...
      // err &&
      //     !this.$refs.user_pw.value.length < 6 &&
      //     ((msg = "비밀번호는 최소 6자 이상입니다."),
      //         (err = false),
      //         this.$refs.user_pw.focus());
    ...
    checkDuplicateId() {
      ...
      http
        .get(`/user/api/idCheck/${this.user_id}`)
        .then((response) => {
